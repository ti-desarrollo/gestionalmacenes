...
class Pedido extends Conexion
{
    ...
    public function listarPedidos_A(string $inicio, string $fin): array
    {
        ...
        foreach ($data as $pedido => &$valor) {
            ...
            foreach ($files as $file) {
                $adjuntos .= "<p style='margin: unset;'><a href='" . LINK_ARCHIVOS . "/{$file['carpeta']}/RECEPCIÓN DE MERCADERÍA - ALMACÉN/{$file['year']}/{$file['mes']}/COMPRAS NACIONALES/{$file['proveedor']}/{$file['fechaFormato']}/{$file['fileName']}' target='_blank'>{$file['fileName']}</a></p>";
            }
    ...
    {
        return $this->returnQuery('sp_listarPedidos ?, ?, ?, ?', [DATABASE_SAP, $sede, $inicio, $fin]);
    }

    public function buscarDetalle(string $sede, string $codigo, string $guia): array
    {
        return $this->returnQuery('sp_buscarPedido ?, ?, ?, ?', [DATABASE_SAP, $sede, $codigo, $guia]);
    }
    ...
    {
        return $this->returnQuery('SELECT U_AMQ_ESTADO_OC estado FROM ' . DATABASE_SAP . '.dbo.OPOR WHERE DocEntry = ?', [$codigo]);
    }
    ...
    private function actualizarCabecera(string $codigo, string $guia, string $estado, string $comentarios, string $conformidad, string $usuario): int | bool
    {
        ...
        $this->simpleQuery('sp_actualizarCabeceraPedido ?, ?, ?, ?, ?, ?, ?, ?', [DATABASE_SAP, $codigo, $guiaDatos[0], $guiaDatos[1], $guiaDatos[2], $estado, $comentarios, $conformidad]);
    ...
    {
        return $this->simpleQuery('sp_actualizarDetallePedido ?, ?, ?, ?, ?', [DATABASE_SAP, $cabecera, $codigo, $itemcode, $cantidad]);
    }

    public function layout(string $codigo): array
    {
        return $this->returnQuery(DATABASE_SAP . '.dbo.SYP_LYT_COMOC01 ?', [$codigo]);
    }

    private function listarArchivos(string $codigo, string $guia): array
    {
        return $this->returnQuery('sp_listarDocumentosPedido ?, ?, ?', [DATABASE_SAP, $codigo, $guia]);
    }
    ...
    {
        foreach ($items as $item) {
            $this->simpleQuery('sp_rollbackPedidoDetalle ?, ?, ?, ?', [DATABASE_SAP, $pedido, $item['item'], $item['cantidadPendienteRecibida']]);
        }
        return $this->simpleQuery('sp_rollbackPedido ?, ?, ?', [DATABASE_SAP, $pedido, $cabecera]);
    }

    public function listarIngresos(int $pedido): array
    {
        $data = $this->returnQuery('sp_listarIngresosPedido ?', [$pedido]);
        foreach ($data as $pedido => &$valor) {
            $adjuntos = '';
    ...
    {
        return $this->returnQuery('sp_buscarDetalleIngresoPedido ?', [$pedido]);
    }
    ...
    {
        foreach ($items as $item) {
            $this->simpleQuery('sp_rollbackPedidoDetalle ?, ?, ?, ?', [DATABASE_SAP, $pedido, $item['item'], floatval($item['cantidad'])]);
        }
        ...
        if ($this->simpleQuery('sp_rechazarRecepcionPedido ?, ?, ?, ?, ?', [DATABASE_SAP, $pedido, $cabecera, $usuario, $comentarios]) > 0) {
